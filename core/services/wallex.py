...
class WallexService(BaseExchangeService):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.background_tasks = [health_task, status_task]
    ...
    async def _status_monitor(self):
        ...
        try:
            while self.is_connected:
                ...
                logger.info(f"{self.exchange_name}: 📊 WALLEX Status Report:")
                logger.info(f"  🔗 Endpoint: https://api.wallex.ir")
                logger.info(f"  ⏱️  Time since last data: {time_since_data:.1f}s")
                logger.info(f"  📝 Messages processed: {self.message_count}")
                logger.info(f"  📡 Broadcaster events: {self.broadcaster_events}")
                logger.info(f"  💰 Market data processed: {self.market_data_processed}")
                logger.info(f"  📡 Subscribed pairs: {len(self.subscribed_pairs)}")
                logger.info(f"  🔌 Socket.IO connected: {self.sio.connected if self.sio else False}")
